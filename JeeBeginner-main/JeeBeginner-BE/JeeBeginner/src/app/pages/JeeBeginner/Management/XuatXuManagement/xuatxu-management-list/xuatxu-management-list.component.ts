...
export class XuatXuManagementListComponent
  implements
  OnInit,
  OnDestroy {
  ...
  ngOnInit(): void {
    ...
    const sb = this.xuatxuManagementService.isLoading$.subscribe((res) => (this.isLoading = res));
  ...
  create() {
    ...
    this.xuatxuManagementService.setIsUpdateMode(true);
  ...
  update(item) {
    ...
    this.xuatxuManagementService.setIsUpdateMode(false);
    dialogRef.afterClosed().subscribe((res) => {
      if (!res) {
        this.xuatxuManagementService.fetch();
        this.xuatxuManagementService.setIsUpdateMode(false);
  ...
  delete(_item: XuatXuModel) {
	...
	deleteDM_LoaiMatHangs() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.xuatxuManagementService.DeletesXuatXu(idsForDeletion).subscribe(res => {
				if (res && res.status === 1)
					this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete, 2000, true, false);
				else
					this.layoutUtilsService.showActionNotification(res.error.message, MessageType.Delete, 0);
