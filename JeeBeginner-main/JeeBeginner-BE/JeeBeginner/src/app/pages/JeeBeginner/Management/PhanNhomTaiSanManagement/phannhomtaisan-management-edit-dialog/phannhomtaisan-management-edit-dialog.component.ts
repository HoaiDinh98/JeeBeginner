...
@Component({
  ...
  templateUrl: "./phannhomtaisan-management-edit-dialog.component.html",
  styleUrls: ["./StylePhongTo.component.css"],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class PhanNhomTaiSanManagementEditDialogComponent
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<PhanNhomTaiSanManagementEditDialogComponent>,
  ...
  ngOnInit(): void {
    ...
    this.loadForm();
    const sb = this.phannhomtaisanManagementService.isLoading$.subscribe((res) => {
      this.isLoading = res;
    });
    this.subscriptions.push(sb);
  this.loadInitData();

  this.phannhomtaisanManagementService.isUpdateMode$.subscribe(value => {
    this.isUpdateMode = value;
  });
  }
  loadInitData() {
    if (this.item.IdPNTS !== 0) {
      const sbGet = this.phannhomtaisanManagementService.gettaikhoanModelByRowID(this.item.IdPNTS).pipe(tap((res: ResultObjModel<PhanNhomTaiSanModel>) => {
        if (res.status === 1) {
          this.item = res.data;
  ...
  loadForm() {
    this.formGroup = this.fb.group({
      MaNhom: ['', Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(50)])],
      TenNhom: ['', Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(50)])],   
  ...
  savecontinute(){
    if (this.formGroup.valid) {
      ...
      this.item.IdPNTS === 0 ? this.create(model,'saveAndContinue') : this.update(model);
    } else {
  ...
  getTitle() {
    if (this.item.IdPNTS === 0) {
      return this.translateService.instant('PHANNHOMTAISANMANAGEMENT.THEMPHANNHOMTAISAN');
    }
    return this.translateService.instant('PHANNHOMTAISANMANAGEMENT.SUAPHANNHOMTAISAN');
  }
  ...
  update(item: PhanNhomTaiSanModel) {
    debugger
    this.isLoadingSubmit$.next(true);
    this.phannhomtaisanManagementService.UpdatePhanNhomTaiSan(item).subscribe((res) => {
      this.isLoadingSubmit$.next(false);
      if (res && res.status === 1) {
        this.dialogRef.close(res);
      } else {
