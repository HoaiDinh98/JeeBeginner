...
import { ChangeDetectionStrategy, ChangeDetectorRef,ElementRef, Component, EventEmitter, HostListener, OnDestroy, OnInit, ViewChild } from '@angular/core';
...
export class MatHangManagementListComponent
  implements
  OnInit,
  OnDestroy {
...
items: any={};
  ...
  @ViewChild(MatPaginator, { static: true }) paginator2: MatPaginator;
	...
	paginatorTotalSubject: BehaviorSubject<number> = new BehaviorSubject<number>(0);
  ...
  displayedColumns = ['CheckBox','stt','HinhAnh','mahang', 'tenmathang', 'soKyTinhKhauHaoToiThieu', 'soKyTinhKhauHaoToiDa',  'thaotac'];
  ...
  ngOnInit(): void {
    ...
    const sb = this.mathangManagementService.isLoading$.subscribe((res) => (this.isLoading = res));
    this.subscriptions.push(sb);
		...
		// merge(this.sort2.sortChange, this.paginator2.page).pipe(
		// 	tap(() => {
		// 		this.loadDM_MatHangsList(true);
		// 	})
		// ).subscribe();
		...
		// // this.subheaderService.setTitle('Mặt hàng');
		// // this.subheaderService.setBreadcrumbs([
		// // 	{ title: 'Danh mục' },
		// // 	{ title: 'Mặt hàng', page: `/dm-mat-hang` },
		// // ]);
		// // Init DataSource
		// this.route.queryParams.subscribe(params => {
		// 	let queryParams = this.mathangManagementService.lastFilter$.getValue(); 
		// 	queryParams.filter=this.filterConfiguration();
		// 	this.paginator2.pageIndex=queryParams.pageNumber;
		// 	// First load
		// 	this.loadDM_MatHangs(queryParams);
		// 	setTimeout(x => { this.loadPage(); }, 500)
		// });
		// this.entitySubject.subscribe(res => {
		// 	this.dm_mathangsResult = res
		// 	this.tmpdm_mathangsResult = []
		// 	if (this.dm_mathangsResult != null) {
		// 		for (let i = 0; i < this.dm_mathangsResult.length; i++) {
		// 			let tmpElement = new MatHangModel();
		// 			tmpElement.copy(this.dm_mathangsResult[i])
		// 			this.tmpdm_mathangsResult.push(tmpElement);
		// 		}
		// 	}
		// });
		// this.layoutUtilsService.setUpPaginationLabels(this.paginator2);
		// //Auto Refresh
		// if (this.autoRefresh)
		// 	interval(10000).subscribe(() => { this.loadDM_MatHangsList(); })
	}
...
//   }
  ...
  create() {
	...
	this.mathangManagementService.setIsUpdateMode(true);
    dialogRef.afterClosed().subscribe((res) => {
      if (!res) {
        this.mathangManagementService.fetch();
		this.mathangManagementService.setIsUpdateMode(true);
      } else {
        this.layoutUtilsService.showActionNotification(saveMessage, messageType, 6000, true, false);
        this.mathangManagementService.fetch();
		this.mathangManagementService.setIsUpdateMode(true);
      }
    });
  ...
  update(item) {
	...
	this.mathangManagementService.setIsUpdateMode(false);
    console.log(item);
    dialogRef.afterClosed().subscribe((res) => {
      if (!res) {
        this.mathangManagementService.fetch();
  ...
  delete(_item: MatHangModel) {
    debugger
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.mathangManagementService.DeleteMatHang(_item).subscribe(res => {
				if (res && res.status === 1) {
	...
	deleteDM_MatHangs() {
		const _title: string = 'Xóa loại mặt hàng';
		const _description: string = 'Bạn có chắc muốn xóa những loại mặt hàng này không?';
		const _waitDesciption: string = 'Loại mặt hàng đang được xóa...';
		const _deleteMessage = `Xóa thành công`;
		const dialogRef = this.layoutUtilsService.deleteElement(_title, _description, _waitDesciption);
		dialogRef.afterClosed().subscribe(res => {
			...
			this.mathangManagementService.DeletesMatHang(idsForDeletion).subscribe(res => {
				if (res && res.status === 1)
					this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete, 2000, true, false);
				else
					this.layoutUtilsService.showActionNotification(res.error.message, MessageType.Delete, 0);
  ...
  filterForm() {
    ...
    this.subscriptions.push(this.filterGroup.controls.donvitinh.valueChanges.subscribe(() => this.filter()));
    this.subscriptions.push(this.filterGroup.controls.loaimathang.valueChanges.subscribe(() => this.filter()));
  }
  ...
  getImageUrl(imageName: string): string {
	...
	loadPage() {
		debugger
		var arrayData = [];
    this.mathangManagementService.isLoading$.subscribe((res) => (this.isLoading = res));
	...
	isVisible(): boolean {
		return this.mathangManagementService.Visible;
	  }
	ListIdLMH() {
    debugger
  ...
  filterIdLMH() {
	debugger
		if (!this.listIdLMH) { return; }
		// get the search keyword
		let search = this.IdLMHFilterCtrl;
		if (!search) { this.filteredIdLMH.next(this.listIdLMH.slice()); return; }
		else { search = search.toLowerCase(); }
		this.filteredIdLMH.next(this.listIdLMH.filter(ts => ts.TenLMH.toLowerCase().indexOf(search) > -1));
		this.changeDetect.detectChanges();
	}
	...
	filterConfiguration(): any {
		...
		if (this.selectIdNhanHieu.length > 0) filter.IdNhanHieu = this.selectIdNhanHieu;
  ...
  ListIdDVT() {
	...
	filterIdDVT() {
		if (!this.listIdDVT) { return; }
		// get the search keyword
		let search = this.IdDVTFilterCtrl;
		if (!search) { this.filteredIdDVT.next(this.listIdDVT.slice()); return; }
		else { search = search.toLowerCase(); }
		this.filteredIdDVT.next(this.listIdDVT.filter(ts => ts.TenDVT.toLowerCase().indexOf(search) > -1));
		this.changeDetect.detectChanges();
	}
  ...
  ListIdNhanHieu() {
	...
	filterIdNhanHieu() {
		...
		this.filteredIdNhanHieu.next(this.listIdNhanHieu.filter(ts => ts.TenNhanHieu.toLowerCase().indexOf(search) > -1));
		this.changeDetect.detectChanges();
	}
	...
	filterIdXuatXu() {
		if (!this.listIdXuatXu) { return; }
		// get the search keyword
		let search = this.IdXuatXuFilterCtrl;
		if (!search) { this.filteredIdXuatXu.next(this.listIdXuatXu.slice()); return; }
		else { search = search.toLowerCase(); }
		this.filteredIdXuatXu.next(this.listIdXuatXu.filter(ts => ts.TenXuatXu.toLowerCase().indexOf(search) > -1));
		this.changeDetect.detectChanges();
	}
  ...
  IsAllColumnsChecked() {
	...
	findIndexColumn(column: string) {
		for (let i = 0; i < this.availableColumns.length; i++) {
			if (this.availableColumns[i].name == column)
				return i;
		}
	}
	...
	dropListDropped(event: CdkDropList, column: string) {
		;
		if (event) {
			moveItemInArray(this.availableColumns, this.previousIndex, this.findIndexColumn(column));
	...
	applySelectedColumns() {
		...
		this.selectedColumns.selected.forEach(col => { _applySelectedColumns.push(col.name) });
