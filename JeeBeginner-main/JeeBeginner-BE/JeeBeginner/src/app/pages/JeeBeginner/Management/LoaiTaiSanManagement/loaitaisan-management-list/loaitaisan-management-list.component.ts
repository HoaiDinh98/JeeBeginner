...
export class LoaiTaiSanManagementListComponent
  ...
  create() {
    ...
    this.loaitaisanManagementService.setIsUpdateMode(true);
  ...
  update(item) {
    ...
    this.loaitaisanManagementService.setIsUpdateMode(false);
    dialogRef.afterClosed().subscribe((res) => {
      if (!res) {
        this.loaitaisanManagementService.fetch();
        this.loaitaisanManagementService.setIsUpdateMode(false);
  ...
  delete(_item: LoaiTaiSanModel) {
    ...
    const _deleteMessageTrung = `Loại tài sản đã không còn hiệu lực`;
