...
const API_PRODUCTS_URL = environment.ApiRoot + '/loaitaisanManagement';
...
@Injectable()
export class LoaiTaiSanManagementService {
  // Private fields
  private _items$ = new BehaviorSubject<LoaiTaiSanModel[]>([]);
  ...
  public fetch() {
    ...
    const request = this.findData(this._tableState$.value)
      .pipe(
        tap((res: ResultModel<LoaiTaiSanModel>) => {
          if (res && res.status === 1) {
            console.log(res);
  ...
  private findData(tableState: ITableState): Observable<any> {
    ...
    const url = API_PRODUCTS_URL + '/LoaiTaiSanList';
  ...
  DM_LoaiTaiSan_Insert(item: LoaiTaiSanModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/create-LoaiTaiSan';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }

  UpdateLoaiTaiSan(item: LoaiTaiSanModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/update-LoaiTaiSan';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
 DeleteLoaiTaiSan(item: LoaiTaiSanModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/delete-LoaiTaiSan';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
	DeletesLoaiTaiSan(ids: any[] = []): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/deletes-LoaiTaiSan';
    return this.http.post<any>(url, ids, { headers: httpHeaders });
	}
  UpdateStatustaikhoan(item: LoaiTaiSanModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
  ...
  gettaikhoanModelByRowID(RowID: number): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + `/GetLoaiTaiSanByRowID?RowID=${RowID}`;
