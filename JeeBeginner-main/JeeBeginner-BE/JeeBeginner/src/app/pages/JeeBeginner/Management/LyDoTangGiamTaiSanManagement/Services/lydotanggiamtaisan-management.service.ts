...
const API_PRODUCTS_URL = environment.ApiRoot + '/lydotanggiamtaisanManagement';
...
export class LyDoTangGiamTaiSanManagementService {
  ...
  private findData(tableState: ITableState): Observable<any> {
    ...
    const url = API_PRODUCTS_URL + '/LyDoTangGiamTaiSanList';
  ...
  DM_LyDoTangGiamTaiSan_Insert(item: LyDoTangGiamTaiSanModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/create-LyDoTangGiamTaiSan';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }

  UpdateLyDoTangGiamTaiSan(item: LyDoTangGiamTaiSanModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/update-LyDoTangGiamTaiSan';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
 DeleteLyDoTangGiamTaiSan(item: LyDoTangGiamTaiSanModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/delete-LyDoTangGiamTaiSan';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
	DeletesLyDoTangGiamTaiSan(ids: any[] = []): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/deletes-LyDoTangGiamTaiSan';
    return this.http.post<any>(url, ids, { headers: httpHeaders });
	}
  UpdateStatustaikhoan(item: LyDoTangGiamTaiSanModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
  ...
  gettaikhoanModelByRowID(RowID: number): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + `/GetLyDoTangGiamTaiSanByRowID?RowID=${RowID}`;
