...
export class XuatXuManagementEditDialogComponent
  implements OnInit, OnDestroy
{
  ...
  this.xuatxuManagementService.isUpdateMode$.subscribe(value => {
    this.isUpdateMode = value;
  });
  }
  loadInitData() {
    if (this.item.IdXuatXu !== 0) {
      const sbGet = this.xuatxuManagementService.gettaikhoanModelByRowID(this.item.IdXuatXu).pipe(tap((res: ResultObjModel<XuatXuModel>) => {
        if (res.status === 1) {
          this.item = res.data;
  ...
  savecontinute(){
    if (this.formGroup.valid) {
      ...
      this.item.IdXuatXu === 0 ? this.create(model,'saveAndContinue') : this.update(model);
    } else {
  ...
  Luu() {
    ...
    if (this.formGroup.valid) {
      const model = this.prepareData();
      this.item.IdXuatXu === 0 ? this.create(model,'save') : this.update(model);
    } else {
      this.validateAllFormFields(this.formGroup);
    }
  }
  getTitle() {
    if (this.item.IdXuatXu === 0) {
      return this.translateService.instant('XUATXUMANAGEMENT.THEMXUATXU');
    }
    return this.translateService.instant('XUATXUMANAGEMENT.SUAXUATXU');
  }
  ...
  reset() {
    debugger
        this.xuatxuManagementService.setTempTennhanhieu(this.formGroup.get('tenxuatxu').value);
