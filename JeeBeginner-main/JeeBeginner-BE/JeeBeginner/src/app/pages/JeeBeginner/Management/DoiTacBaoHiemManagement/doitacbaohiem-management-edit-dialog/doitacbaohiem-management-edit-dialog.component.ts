  ...
  implements OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    this.loadForm();
    const sb = this.doitacbaohiemManagementService.isLoading$.subscribe((res) => {
      this.isLoading = res;
    });
    this.subscriptions.push(sb);
  this.loadInitData();

  this.doitacbaohiemManagementService.isUpdateMode$.subscribe(value => {
    this.isUpdateMode = value;
  });
  }
  loadInitData() {
    if (this.item.Id_DV !== 0) {
      const sbGet = this.doitacbaohiemManagementService.gettaikhoanModelByRowID(this.item.Id_DV).pipe(tap((res: ResultObjModel<DoiTacBaoHiemModel>) => {
        if (res.status === 1) {
          this.item = res.data;
  ...
  loadForm() {
    ...
    if (this.doitacbaohiemManagementService.itemstemp$) {
      ...
      this.formGroup = this.fb.group({
        TenDonVi: [itemstemp.TenDonVi, Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(50)])],
        DiaChi: [itemstemp.DiaChi, Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(50)])],
        SoDT: [itemstemp.SoDT, Validators.compose([Validators.required, Validators.pattern(/^-?(0|[0-9]\d*)?$/), Validators.maxLength(50)])],
        NguoiLienHe: [itemstemp.NguoiLienHe, Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(50)])],
        GhiChu: [itemstemp.GhiChu, Validators.compose([Validators.maxLength(500)])],
      });
    } else {
      this.formGroup = this.fb.group({
  ...
  getTitle() {
    if (this.item.Id_DV === 0) {
      return this.translateService.instant('DOITACBAOHIEMMANAGEMENT.THEMDOITACBAOHIEM');
    }
    return this.translateService.instant('DOITACBAOHIEMMANAGEMENT.SUADOITACBAOHIEM');
  }
  savecontinute(){
      ...
      this.doitacbaohiemManagementService
        ...
        .subscribe((res) => {
          ...
          if (res && res.status === 1) {
            ...
            const dialogRef = this.dialog.open(DoiTacBaoHiemManagementEditDialogComponent, {
              data: { item: item },
              width: '900px',
              disableClose: true,
            });
  ...
  update(item: DoiTacBaoHiemModel) {
    debugger
    this.isLoadingSubmit$.next(true);
    this.doitacbaohiemManagementService.UpdateDoiTacBaoHiem(item).subscribe((res) => {
      this.isLoadingSubmit$.next(false);
      if (res && res.status === 1) {
        this.dialogRef.close(res);
      } else {
