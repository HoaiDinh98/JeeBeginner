...
const API_PRODUCTS_URL = environment.ApiRoot + '/xuatxuManagement';
...
export class XuatXuManagementService {
  ...
  get tempTenXuatXu() {
    return this._tempTenXuatXu;
  }
  setTempTennhanhieu(value: string) {
    this._tempTenXuatXu = value;
  }
  get isLoading$() {
  ...
  private findData(tableState: ITableState): Observable<any> {
    ...
    const url = API_PRODUCTS_URL + '/XuatXuList';
  ...
  DM_XuatXu_Insert(item: XuatXuModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/create-XuatXu';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }

  UpdateXuatXu(item: XuatXuModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/update-XuatXu';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
 DeleteXuatXu(item: XuatXuModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/delete-XuatXu';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
	DeletesXuatXu(ids: any[] = []): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/deletes-XuatXu';
    return this.http.post<any>(url, ids, { headers: httpHeaders });
	}
  UpdateStatustaikhoan(item: XuatXuModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
  ...
  gettaikhoanModelByRowID(RowID: number): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + `/GetXuatXuByRowID?RowID=${RowID}`;
