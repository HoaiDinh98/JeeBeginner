...
const API_PRODUCTS_URL = environment.ApiRoot + '/donvitinhmanagement';
...
@Injectable()
export class DonViTinhManagementService {
  // Private fields
  private _items$ = new BehaviorSubject<DonViTinhModel[]>([]);
  ...
  // get isUpdateMode$(){
  //   return this.isUpdateModeSubject.asObservable();
  // }
  ...
  public fetch() {
    ...
    const request = this.findData(this._tableState$.value)
      .pipe(
        tap((res: ResultModel<DonViTinhModel>) => {
          if (res && res.status === 1) {
            console.log(res);
  ...
  private findData(tableState: ITableState): Observable<any> {
    ...
    const url = API_PRODUCTS_URL + '/DonViTinhList';
  ...
  DM_DonViTinh_Insert(item: DonViTinhModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/create-DonViTinh';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }

  UpdateDonViTinh(item: DonViTinhModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/update-DonViTinh';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
 DeleteDonViTinh(item: DonViTinhModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/delete-DonViTinh';
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
	DeletesDonViTinh(ids: any[] = []): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + '/deletes-DonViTinh';
    return this.http.post<any>(url, ids, { headers: httpHeaders });
	}
  UpdateStatustaikhoan(item: DonViTinhModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
  ...
  gettaikhoanModelByRowID(RowID: number): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + `/GetDonViTinhByRowID?RowID=${RowID}`;
  ...
  exportExcel(queryParams: QueryParamsModel): Observable<Blob> {
		...
		return this.http.get<Blob>(API_PRODUCTS_URL + '/Export_DMLMH', {
			headers: httpHeaders,
			params: httpParams,
