  ...
  implements OnInit, OnDestroy
{
  ...
  this.loaitaisanManagementService.isUpdateMode$.subscribe(value => {
    this.isUpdateMode = value;
  });
  }
  loadInitData() {
    if (this.item.IdLoaiTS !== 0) {
      const sbGet = this.loaitaisanManagementService.gettaikhoanModelByRowID(this.item.IdLoaiTS).pipe(tap((res: ResultObjModel<LoaiTaiSanModel>) => {
        if (res.status === 1) {
          this.item = res.data;
...
debugger
    this.formGroup = this.fb.group({
      MaLoai: ['', Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(50)])],
      TenLoai: ['', Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(50)])],   
  ...
  getTitle() {
    if (this.item.IdLoaiTS === 0) {
      return this.translateService.instant('LOAITAISANMANAGEMENT.THEMLOAITAISAN');
    }
    return this.translateService.instant('LOAITAISANMANAGEMENT.SUATAISAN');
  }
  ...
  savecontinute(){
    if (this.formGroup.valid) {
      ...
      this.item.IdLoaiTS === 0 ? this.create(model,'saveAndContinue') : this.update(model);
    } else {
  ...
  Luu() {
    if (this.formGroup.valid) {
      const model = this.prepareData();
      this.item.IdLoaiTS === 0 ? this.create(model,'save') : this.update(model);
    } else {
      this.validateAllFormFields(this.formGroup);
    }
  }
